...
var isochrone = {
  ...
  load: function (parameters) {
    ...
    this.map.zoom = parameters.zoom || 12;
  ...
  compute: function (parameters) {
    console.log('entring in compute function)');
    ...
    if (!this.ready) {
      return this.computationCallback(
        'KO',
        'Not ready, call init() or wait for init to be callbacked'
      );
    }
    ...
    if (this.computation.slices < 4 || this.computation.slices > 25) {
      return this.computationCallback(
        'KO',
        'Wrong value for slices: between 2 and 25'
      );
    }
    ...
    if (this.computation.cycles < 2 || this.computation.cycles > 50) {
      return this.computationCallback(
        'KO',
        'Wrong value for cycles: between 2 and 50'
      );
    }
    ...
    if (
      !parameters.mode ||
      !parameters.mode.match(/^(walking|bicycling|driving|transit)$/)
    ) {
    ...
    if (
      !parameters.type ||
      (parameters.type !== 'duration' && parameters.type !== 'distance')
    ) {
  ...
  cycle: function () {
    ...
    isochrone.service.getDistanceMatrix({
      ...
      function (data, result) {
        ...
        if (
          typeof data.rows[0].elements === 'undefined' ||
          data.rows[0].elements.length !== destinations.length
        ) {
